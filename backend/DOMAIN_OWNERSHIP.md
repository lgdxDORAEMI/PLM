...
- `FUC-H-INVITE-001`: 초대 수락 화면이 미확정이므로 수락 Router는 추가하지 않았다. Account Service port 확장 전 화면·인증 복귀 계약을 확정해야 한다.
- `FUC-W-COND-003`과 UC2 A1의 설명이 충돌한다. 최신 기능 요구사항은 남편 변경 알림 발송으로 확정했지만 유스케이스 문서는 미정으로 남아 있다. Skeleton은 Notification type만 예약하고 자동 생성은 구현하지 않았다.
