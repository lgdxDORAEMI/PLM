...
# LG COMMAND_NOT_SUPPORTED_IN_STATE / COMMAND_NOT_SUPPORTED_IN_POWER_OFF.
# 기기가 이미 요청한 전원 상태면 LG가 이 코드들로 거부한다(이미 켜진 기기에 "켜기",
# 이미 꺼진 기기에 "끄기") — 실패로 취급하면 뒤이은 바람세기 명령까지 막힌다.
_ALREADY_IN_STATE = {"2302", "2304"}
...
async def control_device(
    ...
    if body.power is not None:
        ...
        if result.status == ControlStatus.ERROR and result.error_code in _ALREADY_IN_STATE:
