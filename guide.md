...
컨디션 캘린더는 월 목록을 먼저 표시하고 선택 날짜의 상세를 `GET /api/v1/care/calendar/days/{target_date}`로 별도 조회합니다. 상세 조회가 실패해도 월 목록은 유지되며 상세 영역에서 다시 시도할 수 있습니다. 이 통합 조회는 기존 컨디션과 리포트 데이터를 읽기만 하므로 별도 DB 마이그레이션이 필요하지 않습니다.
