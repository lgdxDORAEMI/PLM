...
Supabase와 Backend가 연결된 앱은 아내 계정으로 자동 진입합니다. 아내·남편 메뉴의 `계정 전환` 버튼은 로그인 화면 없이 두 계정의 실제 인증 세션과 화면을 전환합니다. 계정 비밀번호는 Backend의 비공개 설정에만 보관하며, 이 경로는 같은 PC의 localhost에서만 사용합니다.
...
아내 메뉴의 `초기화`는 로그인한 아내의 KST 오늘 컨디션, 모든 루틴 revision, 실행·피드백, 가사 요청과 리포트를 한 트랜잭션으로 삭제한 뒤 컨디션 입력 화면으로 이동합니다. 남편 화면은 재진입하거나 앱으로 복귀할 때 연동된 아내의 기록을 다시 조회합니다.
...
| 임산부 프로필 | 구현 | Supabase 로그인 환경에서 Backend 단계별 API로 출산예정일·마지막 생리 시작일·생년월일·신체정보를 저장하고 조회함. Mock 실행은 브라우저 저장소를 사용함 |
...
4종 상세 가이드는 하나의 Dashboard Template을 복제하지 않습니다. Meal은 끼니별 추천·근거·수락/다른 메뉴 순환을 제공하며, 다른 메뉴를 선택해도 상단의 추천 근거는 유지하고 메뉴 카드만 교체합니다. Household는 직접 할 일을 간단한 목록으로 표시하고, 가족과 나누기만 체크박스 카드로 선택합니다. 공유한 일은 직접 할 일 목록에서 제외되며 Partner Request 상태로 이어집니다. Health는 부담 부위 우선순위와 활동 완료, Sleep은 취침 맥락·환경 항목별 설정·수면 팁에 각각 최적화되어 있습니다. 수면 환경 전체 실행 버튼은 ‘AI가 맞춘 오늘의 수면 환경’ 제목 옆에 강조색 원형 재생 버튼으로 표시합니다. 오늘의 컨디션 입력 화면에는 기분 항목을 표시하지 않습니다. Backend 저장 계약에 필요한 기분 값은 기존 값 또는 기본값으로 유지합니다. 가사 가전 항목의 `실행`과 수면 환경 전체 실행은 요청할 때마다 오늘 날짜의 가전 실행 이력 1회로 기록하고 확인 팝업을 표시합니다. Daily 리포트와 캘린더는 두 가이드의 이력을 합산해 건수와 항목을 표시합니다. 현재 가전 동작은 로컬 기록이며 실제 기기 제어와 연결되지 않습니다. 사용자 화면에는 개발 단계 안내 문구를 표시하지 않습니다.
...
Supabase 설정이 있는 실행 환경에서는 기존 계정으로 로그인한 뒤 FastAPI의 계정 상태 조회를 통해 아내·남편 화면으로 이동합니다. 아내 프로필의 단계별 저장, 오늘 컨디션 저장·복원, 예정 활동 저장, 루틴 생성·조회, 식사·가사·수면 가이드 조회, 가사 요청 생성, 리포트와 캘린더 조회·확정을 API로 처리합니다. 남편 초대 화면은 항상 기존 레이아웃을 보여주며, 초대장 전송 버튼을 누르면 ThinQ 전달 연동이 필요하다는 안내를 표시합니다. 환경 값이 비어 있으면 기존 로컬 화면 흐름을 사용합니다.

연동 설정이 없는 실행에서는 남편 초대뿐 아니라 가이드·기록·리포트·캘린더·움직임·남편 기능의 기본 화면도 유지합니다. 이때 화면 상단의 `연동이 필요합니다` 안내는 보이는 내용이 연결 전 예시임을 알리며, 실제 API 요청이 실패한 경우에는 예시 데이터로 대체하지 않고 오류 상태를 표시합니다.

마지막 생리 시작일을 선택하면 출산예정일 입력칸에 280일 뒤 계산값을 즉시 표시합니다. 출산예정일을 직접 선택하면 그 값을 우선 사용합니다. Supabase 로그인 환경에서는 생년월일도 프로필 신체정보 API에 저장하고 조회합니다. 가이드 응답의 `item_id`를 사용해 건강 활동 완료와 수면 환경 변경을 서버에 저장합니다. 루틴 생성 결과의 `source`가 `ai`가 아니면 서버가 생성한 기본 루틴으로 표시하며, API 실패 시 Mock 데이터를 대신 표시하지 않습니다. 실제 가전 제어와 Chat 기능은 연결 대상에 포함되지 않습니다. 실행 설정과 점검 방법은 [guide.md](guide.md)를 참고하세요.

예정 활동을 제출하면 `AI 루틴 생성중...` 창과 로딩 애니메이션이 나타납니다. 생성 요청이 완료되면 창이 자동으로 닫히고 홈으로 이동하며, 실패하면 창을 닫고 입력값을 유지한 채 다시 시도할 수 있습니다.
