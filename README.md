...
조회 데이터가 없는 화면을 점검하려면 상단의 현재 화면 제목을 1초 이내 간격으로 5회 연속 선택합니다. 홈·캘린더·실시간·가이드·리포트·알림·가사 요청 화면이 빈 상태 미리보기로 전환되며, 같은 동작을 한 번 더 수행하면 기존 Local Mock 데이터 화면으로 돌아옵니다. 이 기능은 런타임 표시만 전환하므로 기존 Mock 데이터는 삭제하거나 변경하지 않습니다.
