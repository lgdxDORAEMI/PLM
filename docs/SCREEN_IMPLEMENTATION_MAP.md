...
이 문서는 Flutter 제품 화면을 Task 단위로 구현하기 위한 화면-이미지-Route-Component 연결 계획이다. 실제 UI 구현은 포함하지 않는다.
...
현재 `W-PROFILE-001` 임산부 프로필 설정/수정 화면은 `IMPLEMENTED`이며, 나머지 19개 제품 화면은 `SKELETON`이다.
...
| C-06 | 가사·수면 PNG에는 실제 ThinQ 가전 실행 Action이 있으나 MVP는 실제 기기 제어를 제외한다. | W-HOUSE-001, W-SLEEP-001 | MVP에서는 추천·선택·Mock 완료 상태만 구현할지 확인 |
...
| Feature | `condition` |
| 구현 상태 | `SKELETON` |
| 사용해야 하는 공용 컴포넌트 | `TopAppBar`, `SelectionCard`, `AppButton`, Design Token |
| 추가로 필요한 컴포넌트 | `ProgressMetric`, 부위별 `ConditionMetricRow`, `ConditionSelector`, Form validation, 이탈 확인 Dialog |
| 이전 화면 | `/wife/home` |
...
| Flutter 파일 | `frontend/lib/features/home/screens/wife_home_screen.dart` |
| Route | `/wife/home` |
| Feature | `routine` |
| 구현 상태 | `SKELETON` |
| 사용해야 하는 공용 컴포넌트 | `TopAppBar`, `AppBottomNavigation`, `AppCard`, `AppButton` |
| 추가로 필요한 컴포넌트 | `PregnancyWeekHero`, `DailyCareSection`, `RoutineGuideCard`, `RoutineGenerationState`, `CategoryBadge`, `AppSkeleton`, `InfoBanner` |
| 이전 화면 | Bootstrap, 배우자 초대 종료, 예정 활동 저장 |
| 다음 화면 | 컨디션, 식사·가사·건강·수면, Daily 리포트, 전역 프로필 메뉴 |
| Interaction | 컨디션 미입력/입력 완료 조건부 UI, 가이드 진입, 컨디션 수정, 루틴 생성 Loading/Fallback/Retry, 하루 끝내기 |
| 비고 | 관련 ID: `W-ROUTINE-001`, `W-ROUTINE-002`, `W-ROUTINE-003`. C-01, C-03 적용 확인 |
...
| 추가로 필요한 컴포넌트 | `MealPeriodSelector`, `MealRecommendationCard`, `MealCautionSection`, `CategoryBadge`, Evidence row, 이미지 Placeholder 정책 |
...
| 비고 | 관련 ID: `W-MEAL-001`, `W-MEAL-002`, `W-MEAL-003`, `W-MEAL-004`, `W-RECORD-001`. C-01, C-03 적용 확인 |
...
| 비고 | 관련 ID: `W-MOTION-001`, `H-MOTION-001`. 기존 `MovementScreen` 데모와 별도. C-08 확인 전 상세 UI 구현 보류 |
...
| 구현 상태 | `SKELETON` |
| 사용해야 하는 공용 컴포넌트 | `TopAppBar`, `AppBottomNavigation`, `AppCard`, `AppButton`, `SelectionCard` |
| 추가로 필요한 컴포넌트 | `SleepEnvironmentSelector`, `SleepTipList`, 값 편집 Control, Toggle/Checkbox, 완료 상태 Banner |
| 이전 화면 | `/wife/home` |
| 다음 화면 | 완료 후 현재 화면 또는 `/wife/home`; 결과는 Daily 리포트에 반영 |
| Interaction | 환경 항목 복수 선택, 값 수정, 추천값 복원, 루틴 시작, Mock 완료/실패 상태 |
| 비고 | 관련 ID: `W-SLEEP-001`, `W-SLEEP-002`, `W-RECORD-001`. C-06 확인 필요 |
...
| Feature | `meal` |
| 구현 상태 | `SKELETON` |
| 사용해야 하는 공용 컴포넌트 | `TopAppBar`, `AppBottomNavigation`, `AppButton` |
| 추가로 필요한 컴포넌트 | `MealChatConversation`, `ChatBubble`, Message composer, Quick reply, `MealRechoiceCard`, Typing/Error state |
| 이전 화면 | 식사 가이드 또는 Wife Chat Tab |
| 다음 화면 | 추천 적용 시 `/wife/home/meal` |
| Interaction | 메시지 작성·전송, 빠른 답변, 응답 대기·실패, 재추천 반복, 추천 적용, Draft 이탈 확인 |
| 비고 | 관련 ID: `W-MEAL-002`, `W-CHAT-001`, `W-CHAT-002`. C-05 확인 전 식사 외 대화 범위 확장 금지 |
...
공통 기반 준비 이후 `W-PROFILE-001`을 실제 UI로 구현했으며, 그 외 제품 화면은 `SKELETON`으로 유지한다.
