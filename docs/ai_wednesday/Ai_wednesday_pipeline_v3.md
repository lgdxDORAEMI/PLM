...
| 2026-09-22 | (커밋 전) 팀 결정으로 홈 카드 요약 AI 생성 되돌림: 홈은 맞춤 정보가 필요 없고 상세 화면에서만 AI 정보를 쓴다. `prompt.py`·`generator.py`·`service.py`를 `28c25cf` 상태로 복원(`PROMPT_VERSION=2026-09-20.1`, 가이드 AI 출력은 요약 추가 전과 같음). `home.summaries`는 `fallback.yaml` 고정 문구 4개(일반 문구). 주의 문구(`home.caution`)는 그대로(오늘의 팁 우선, 없으면 주차별 문구). 전체 260개 통과, AI 호출 없음 |
