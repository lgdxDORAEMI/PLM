...
| 2026-09-22 | 기분(`mood`) 입력 제외(커밋 전): 기분은 수치화해서 케어할 대상이 아니라는 회의 결정, 원격 `4d94b39`에서 컨디션 화면의 기분 항목 제거. 루틴 입력(`inputs.py` `CONDITION_COLUMNS`)·LLM 전달 키(`service.py` `LLM_FACT_KEYS`)·영향 판단(`impact_map.yaml`)에서 빼고, 쓸 곳이 없어진 척도 반전(`higher_is_worse`)도 삭제. `daily_conditions.mood` 컬럼과 Care 저장 계약은 유지. 테스트: 기분 반전 1개 삭제, 기분 무시 1개 추가, Care 계약 테스트 1곳 수정, 전체 218개 통과. AI·DB 호출 없음, 비용 0. QA '컨디션 조정 API에서 mood 값 고정'은 A안으로 닫음: Care API·DB의 `mood` 필수 조건과 프론트 기본값 4는 유지하되, 루틴·요약 어디서도 읽지 않아 영향 없음 |
| 2026-09-22 | (커밋 전) ① 루틴 생성 실패 대응: 실DB `daily_routines` 09-22 14:54 `fallback_prev` + `ReadError: [WinError 10035]`(팀원 Windows). 원인 추정 = RAG 검색 RPC 4회를 스레드 4개로 동시에 돌려 supabase-py HTTP/2 연결 1개를 공유. `retriever.retrieve`를 스레드 1개 순차 실행으로 변경, 실측 0.31~0.40초(임베딩 포함 3회). Windows 재현·확인은 팀원 몫. ② 홈 주차 특징 블록: `week_notes.yaml`(4주×10구간, 2줄+주의 1줄, 팀 검수 전 초안) + `home.py`, `GET/POST /routine/today` 응답에 `home: {week, week_notes, caution}`(caution = 오늘의 팁 우선). 테스트 4개 추가, 전체 258개 통과. 실DB GET 읽기 확인: testwife 출산예정일 2027-07-08(초기화 후 재입력)이라 week 0 → 1~4주 구간 표시. AI 호출 없음(임베딩 측정 3회 제외), 비용 약 0 |
| 2026-09-22 | (push `d4131cb`, 아래 행에서 되돌림) 홈 카드 4장 요약 `summaries`(A안): 가이드별 생성·수정 호출 스키마에 `summary`(20자 안팎) 추가, `PROMPT_VERSION=2026-09-22.1`. 폴백·요약 없는 가이드는 `fallback.yaml` `summaries` 고정 문구, 수정 경로는 다시 만든 가이드만 새 요약. 저장은 `daily_routines.response.summaries`(DB 변경 없음), 응답은 `home.summaries`. 테스트 2개 추가·3개 기대값 수정, 전체 260개 통과. 실DB GET 읽기 확인(16:58 생성 루틴이라 템플릿 문구). AI 생성 요약은 다음 실제 생성 때 확인. 홈을 카드 4장으로 그리는 것은 프론트 몫 — `docs/api.md`에 요청 |
| 2026-09-22 | (push `eff0e76`) 팀 결정으로 홈 카드 요약 AI 생성 되돌림: 홈은 맞춤 정보가 필요 없고 상세 화면에서만 AI 정보를 쓴다. `prompt.py`·`generator.py`·`service.py`를 `28c25cf` 상태로 복원(`PROMPT_VERSION=2026-09-20.1`, 가이드 AI 출력은 요약 추가 전과 같음). `home.summaries`는 `fallback.yaml` 고정 문구 4개(일반 문구). 주의 문구(`home.caution`)는 그대로(오늘의 팁 우선, 없으면 주차별 문구). 전체 260개 통과, AI 호출 없음 |
| 2026-09-22 | (push `11888d2`) ① 식사 4끼 필수: 지시문 "아침·점심·저녁·밤(snack) 각 1개, 반드시 4개", `fallback.yaml`에 `meal:snack` 추가(18:11 수정에서 AI가 snack을 빼 3끼가 된 사례). 프론트 표시 이름 '간식'→'밤'(사용자 승인 예외). ② 수면 환경 type: 한글("조명")로 저장돼 프론트가 5종 모두 조명으로 처리 → 스키마 enum 코드 5종(`SLEEP_ENV_TYPES`), 예전 행은 `/sleep/today` 조회 때 코드로 변환(`guide/query_service.py`). 잘못 저장된 `sleep_env_override`(`lighting: 24도`, `40%`) 2건은 그대로 둠. `PROMPT_VERSION=2026-09-22.2`. 테스트 2개 추가·3곳 기대값 수정, 전체 264개 통과. 실DB `/sleep/today` 읽기로 코드 변환 확인 |
| 2026-09-22 | (커밋 전) 4끼 이전 루틴 보정: 컨디션 수정 경로에서 직전 루틴이 `prompt_version < 2026-09-22.2`이고 끼니가 빠졌으면 입덧이 그대로여도 식사를 다시 만든다(`service._missing_meals`, `MEAL4_PROMPT_VERSION`). 새 버전에서 알레르기 검증으로 끼니가 빠진 경우·폴백 행(prompt_version 없음)은 제외해 매 수정마다 재생성하지 않는다. `get_edit_base`가 `prompt_version`도 읽음. 프론트 홈 '이번 주에 알아두세요'를 `home.week_notes`·`home.caution`에 연결(`PregnancyWeekTipCard`, 팀 합의로 프론트 직접 수정). 테스트 백엔드 2개·프론트 단언 2개 추가, 백엔드 266개 통과, 프론트는 기존 실패 1건 외 통과 |
